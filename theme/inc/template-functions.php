...
function bluebox_html5_comment( $comment, $args, $depth ) {
	...
	<<?php echo esc_attr( $tag ); ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $comment->has_children ? 'parent' : '', $comment ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body bg-gray-50 border border-black/5 rounded-sm my-2 pt-4 pr-4">
			<footer class="comment-meta flex items-center gap-2">
				...
				<div class="comment-metadata text-xs">
					<?php
					echo shamsi_date('d F, Y', get_comment_time('U'));
					printf(
						'<a class="hidden" href="%s"><time datetime="%s">%s</time></a>',
